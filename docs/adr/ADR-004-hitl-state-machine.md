...
- 状态：已接受（Accepted）
- 日期：2026-08-27
- 关联：DESIGN.md L3 AI Runtime；传统总账实证（填制→审核→记账→结账状态机与本模型同构；逆向取消=补偿）

## 背景

凭证是财务系统的核心聚合。AI 参与后必须回答：哪些动作 Agent 可以自己做、哪些必须人审、出错如何回退。传统总账 用 30 年验证的答案：状态机 + 逆向窗口（未结账才可逆）。
...
- 正面：Agent 权限边界即数据（可配置、可审计）；与 传统总账 用户心智模型兼容（迁移零学习成本）。
