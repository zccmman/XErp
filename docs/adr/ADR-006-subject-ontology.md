...
## 表单消费：制单页辅助维度输入（阶段1，2026-09-08）

本体接线（HITL）后有个实际摩擦：Web 制单表单没有辅助维度输入，
录应收/应付时 `aux_required` 每次都命中 → 会计每次都要勾一次
「已知晓」。提示本该引导用户**补上**维度，而不是教他**习惯**确认框。

方案（`kernel/webapp.py`）：

- 表单每行加「辅助维度」输入，格式 `customer=甲公司`（k=v，
  与 coa attrs 同风格，分号分隔多个）；
- 前端把科目 `aux_dim_defs` 内嵌为 `ACCT_DIMS` 映射，选科目后
  placeholder 就地提示「须挂：customer（格式 customer=值）」——
  提示发生在用户输入之前，而不是提交之后的确认框里；
- 提交解析失败（缺 `=`）原地报错回填；维度进入本体预检
  （挂对维度 → `aux_required` 不再命中 → 无需确认）与内核建单；
- 双层防线保持：预检只建议（可勾选继续），内核
  `AUX_DIM_UNDECLARED` 校验兜底——表单放行不了内核不放行的东西。

值得一提：此前 HITL 用例假设「往来科目必弹确认」，
本步落地后该假设被新输入项推翻——挂对维度即直通。
