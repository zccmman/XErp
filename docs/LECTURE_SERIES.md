...
```
逆向 U8 ──► 提炼治理原语 ──► 设计 LedgerOS ──► TDD 实现 ──► 真机验证 ──► 开源路线
...
**学习目标**：能独立画出 LedgerOS 分层架构并解释每条铁律的「为什么」。
...
- 待决：D4 代号 LedgerOS / D3 GitHub 私有仓
...
- C. 今日时间线：reverse-skill 安装 → U8 介质解析 → 知识库语料 → GL 逆向 → LedgerOS 立项 → P0-01~09
