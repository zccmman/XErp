...
| `驳回 <凭证号> <意见>` | PUSHED → DRAFT | 意见入 `VOUCHER_REJECTED` 事件（**意见必填**，缺则退回提示） |
| `帮助` / `help` | 指令说明 | — |
| 卡片按钮「批准/驳回」 | 同上（驳回意见固定占位「（企微卡片驳回，未填意见）」，可补文本指令） | 同上 |

> 若驳回执行人恰是制单人本人，内核会记为 `VOUCHER_WITHDRAWN`（撤回）而非驳回——
> 同一个 PUSHED→DRAFT 跃迁按执行人区分事件类型，避免「被退回」与「自己收回」在审计上混为一谈。
