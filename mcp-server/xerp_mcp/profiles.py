...
三档：
    ...
    standard 标准  48 个：+ 驳回/撤回/反记账/多级签字/月结/往来/银行/OCR/三表预测/专项核算/账本精灵主动推送/AI风险预警/报税准备/审计追踪/票据入账预览门禁/应收应付对账单/应收应付账龄/应收应付未清项/核销草稿
    pro      专业  77 个：全量（自治授权/过账、异常扫描、适配器、转账模板、双审批通道、多主体合并报表、合并血缘下钻、合并层级标注、内部往来配对草稿、长投权益抵销草稿、合并现金流量表、合并现金流抵消草稿、应收应付核销落地）
...
STANDARD_EXTRA: tuple[str, ...] = (
    ...
    # 往来与资金
    "partner_balances",
    # 应收应付深化（对账单 + 账龄 + 未清项 + 核销草稿）：与 partner_balances 同一
    # 取数口径，单一真源；未清项/核销草稿只读、零风险
    "arap_statement",
    "arap_aging",
    "arap_open_items",
