...
三档：
    ...
    standard 标准  46 个：+ 驳回/撤回/反记账/多级签字/月结/往来/银行/OCR/三表预测/专项核算/账本精灵主动推送/AI风险预警/报税准备/审计追踪/票据入账预览门禁/应收应付对账单/应收应付账龄
    pro      专业  72 个：全量（自治授权/过账、异常扫描、适配器、转账模板、双审批通道、多主体合并报表、合并血缘下钻、合并层级标注、内部往来配对草稿、长投权益抵销草稿）
...
STANDARD_EXTRA: tuple[str, ...] = (
    ...
    # 往来与资金
    "partner_balances",
    # 应收应付深化（对账单 + 账龄）：与 partner_balances 同一取数口径，单一真源
    "arap_statement",
    "arap_aging",
    "bank_import_csv",
    "bank_reconcile",
...
PROFILES: dict[str, str] = {
    ...
    "pro": "专业：全量，含自治过账、风控、适配器、转账模板、双审批通道、合并报表增强",
}
